...
void            pt_requestor_free (PtRequestor * requestor);

typedef struct _PtMultiPageRequestData PtMultiPageRequestData;

typedef         gboolean(*PtSendRequestMultiPageSuccessFunc) (PtRequestor * r, xmlnode * node, gboolean last_page, PtMultiPageRequestData * request, gpointer user_data);
typedef         gboolean(*PtSendRequestMultiPageErrorFunc) (PtRequestor * r, const PtRequestorErrorData * error_data, gpointer user_data);

struct _PtMultiPageRequestData {
    gpointer        user_data;
    char           *url;
    //char *query_string;
...
//void            pt_send_xml_request_multipage_all(PtRequestor * r, const char *url, PtRequestorParams * params, PtSendRequestMultiPageAllSuccessFunc success_callback, PtSendRequestMultiPageAllErrorFunc error_callback, int expected_count, gint max_count, gpointer data);

/* statuses/friends API deprecated page based retrieval,
 * and use cursor based method instead */
//void            pt_send_xml_request_with_cursor(PtRequestor * r, const char *url, PtRequestorParams * params, long long cursor, PtSendRequestMultiPageAllSuccessFunc success_callback, PtSendRequestMultiPageAllErrorFunc error_callback, gpointer data);
...
//const gchar    *pt_response_text_data(const gchar * response_text, gsize len);
//gint            pt_response_text_status_code(const gchar * response_text);
